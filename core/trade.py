...
def evaluate_order(order_tx: pb2.OrderTx, grid_rate: pb2.GridRateTx, height: int):
    ...
    if order_tx.type == pb2.PUSH:
        if grid_rate.push_rate < order_tx.limit_price:
            print(f"Order PUSH limit failed: {grid_rate.push_rate} < {order_tx.limit_price}")
            return False
    else:
        # consumer will buy if the grid pull rate <= limit price
        if grid_rate.pull_rate > order_tx.limit_price:
            print(f"Order PULL limit failed: {grid_rate.pull_rate} > {order_tx.limit_price}")
            return False
...
def create_trade_tx(order_tx: pb2.OrderTx, grid_rate: pb2.GridRateTx, miner_address: str, fee: int) -> pb2.Transaction:
    trade = pb2.TradeTx()
