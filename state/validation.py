...
def validate_order_tx(tx, ctx, state):
    ...
    if order.type == pb2.PUSH:
        if gr.push_rate < order.limit_price:
            return False, f"OrderTx PUSH limit {order.limit_price} not met by grid push_rate {gr.push_rate}"
        
    elif order.type == pb2.PULL:
        if gr.pull_rate > order.limit_price:
            return False, f"OrderTx: PULL limit {order.limit_price} not met by grid pull_rate {gr.pull_rate}"
    ...
    if order.type == pb2.PUSH:
        if account.energy_wh < order.energy_wh:
            return False, f"OrderTx: insufficient energy for {order.sender_address[:16]}, has {account.energy_wh} wh, needs {order.energy_wh} wh"
        
    else:
        settlement = order.energy_wh * gr.pull_rate
        if account.micro_coins < settlement:
            return False, f"OrderTx: insufficient coins for {order.sender_address[:16]}, has {account.micro_coins}, needs at least {settlement}"
...
def validate_trade_tx(tx, ctx, state):
    ...
    if trade.grid_rate.push_rate != ctx.grid_rate.push_rate or \
       trade.grid_rate.pull_rate != ctx.grid_rate.pull_rate:
        return False, f"TradeTx: embedded grid_rate (push={trade.grid_rate.push_rate}, pull={trade.grid_rate.pull_rate}) does not match active rate (push={ctx.grid_rate.push_rate}, pull={ctx.grid_rate.pull_rate})"
    ...
    if trade.settlement_amount != expected_settlement:
        return False, f"TradeTx settlement_amount {trade.settlement_amount} not the expected {expected_settlement}"
    ...
    if order.type == pb2.PUSH:
        if user.energy_wh < order.energy_wh:
            return False, f"TradeTx: user {order.sender_address[:16]}… has {user.energy_wh} wh, needs {order.energy_wh} wh"
       
        if grid.micro_coins < trade.settlement_amount + trade.miner_fee:
            return False, f"TradeTx: grid has {grid.micro_coins} coins, needs {trade.settlement_amount + trade.miner_fee}"
        
    else:
        total_cost = trade.settlement_amount + trade.miner_fee
        if user.micro_coins < total_cost:
            return False, f"TradeTx: user {order.sender_address[:16]}… has {user.micro_coins} coins, needs {total_cost}"
        
        if grid.energy_wh < order.energy_wh:
            return False, f"TradeTx grid has {grid.energy_wh} wh, needs {order.energy_wh} wh"
...
def validate_grid_rate_tx(tx, ctx, state):
    ...
    if gx.push_rate <= 0:
        return False, f"GridRateTx must have a positive push_rate, but got {gx.push_rate}"
    
    if gx.pull_rate <= 0:
        return False, f"GridRateTx must have a positive pull_rate, but got {gx.pull_rate}"
    
    if gx.push_rate > gx.pull_rate:
        return False, f"GridRateTx push_rate ({gx.push_rate}) must be less than or equal to the pull_rate ({gx.pull_rate})"
    
    if gx.expiry_height <= ctx.height:
        return False, f"GridRateTx rate expired at height {gx.expiry_height}, current height {ctx.height}"
    
    if gx.grid_address != GRID_ADDRESS:
        return False, f"GridRateTx: grid_address mismatch, expected {GRID_ADDRESS}, got {gx.grid_address}"
